...
from app.model.agent_event import AnswerEvent, BaseAgentEvent, QuestionEvent, TextContent
...
class AgentWorkflow:
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            self.history.append(AnswerEvent.create(content=TextContent(text=response)))
            await self.next_event()

    @workflow.update
    async def ask_agent(self, question: str) -> str:
        workflow.logger.info("ask_agent called")
        self.history.append(QuestionEvent.create(content=TextContent(text=question)))
        while True:
    ...
    async def next_event(self) -> BaseAgentEvent:
